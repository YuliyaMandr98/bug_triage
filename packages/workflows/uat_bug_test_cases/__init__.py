...
heading parser, so an LLM (`GeminiClient.extract_bug_repro_steps`) extracts
...
async def run_uat_bug_test_cases_workflow(
    ...
    *,
    ...
    for idx, issue in enumerate(new_bugs, 1):
        ...
        try:
            extracted = await gemini_client.extract_bug_repro_steps(key, summary, description_text)
        except Exception as exc:
