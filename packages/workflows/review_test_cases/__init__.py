...
# (see _MAX_REVIEW_FIELD_CHARS in packages/integrations/gemini) - used here only
# to surface a visible warning in the run log when a field is about to be
...
async def run_review_test_cases_workflow(
    ...
    *,
    ...
    _log("INFO", "Отправляю на анализ в Gemini…")

    try:
        review = await gemini_client.review_test_case_coverage(
            test_type=test_type,
            us=us,
            spec_text=spec_text,
