...
Respond with valid JSON only (no markdown). Write the "reasoning" value in
RUSSIAN, regardless of the language of the specification/bug text above -
this field is read directly by a Russian-speaking QA team:
{{
...
If NOT a real bug, set priority to "Low" and explain why in reasoning (in Russian)."""
    ...
    async def extract_bug_repro_steps(self, bug_key: str, summary: str, description_text: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Gemini bug repro-steps extraction failed for {bug_key}: {str(e)}")
    ...
    async def classify_skipped_tests(self, batch: list[dict[str, Any]]) -> list[dict[str, Any]]:
            ...
            return [
                {
                    ...
                    "category": "unclear",
                    "bug_keys": [],
                    "reason_summary": "Gemini API key not configured",
                }
                for item in batch
            ]
        ...
        try:
            ...
            logger.error(f"Gemini skipped-test classification failed: {str(e)}")
