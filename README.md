...
Плюс read-only **Skipped Tests Audit**: сканирует автотесты (*.spec.ts) на предмет `.skip`,
...
минимум с одной задачей типа «История» и как минимум с одной задачей типа «QA» — без Gemini,
...
разобранные Gemini шаги воспроизведения из бага. Дубликаты не создаются — уже представленные
...
1. Откройте **http://localhost:8000/ui/workflows/skipped_tests_audit/run**.
...
3. Воркфлоу рекурсивно сканирует все `*.spec.ts`, находит тесты с `.skip`, комментарием
   `todo`/`жду` или ссылкой на баг (`MB-XXXX`/`МВ-ХХХХ`), просит Gemini разобраться в
   настоящей причине каждого пропуска (комментарий рядом с тестом не всегда описывает
   причину именно этого теста — Gemini отличает актуальную причину от случайных
...
Только чтение — ничего не пишет и не меняет в Jira. Без Gemini — чистая проверка по правилам.

1. Откройте **http://localhost:8000/ui/workflows/bug_backlog_audit/run**.
2. Укажите JQL (по умолчанию — все баг-тикеты в статусе Backlog) и максимум багов для проверки.
3. Для каждого найденного бага воркфлоу проверяет два отдельных типа замечаний (никогда не смешиваются в один список):
   - **Недостающие поля** — должны быть заполнены, но пусты: **Фаза**, **Метки**, **Компоненты**,
     **ENV (Полигон)**, **Team**.
   - **Лишние поля** — заполняются только когда баг уже берётся в работу, а для бэклог-бага должны
     быть пустыми: **Исходная оценка** (обнулённое значение не считается заполненным), **Sprint**,
     **Available at Android build**, **Available at iOS build**, **Available at WEB app build**,
     **Available at AP WEB build**, **Available at AP BE build**, **Available at BE build**.
   - **Недостающие связи** — связь «is Bug for» или «blocks» как минимум с одной задачей типа
     **«История»** (User Story) и как минимум с одной **QA task** (любой тип задачи, содержащий
     «QA» в названии — на этом Jira-инстансе их несколько на команду/платформу, например
     «QA MB task», «QA WEB auto task», «QA API task»).
   - **Лишние связи** — связь «клонирует задачу» (Cloners, outward-фраза «clones») на другую задачу
     не должна присутствовать.
4. Пример бага с корректно заполненными полями и связями «История»/«QA» —
   [MB-6419](https://fincabank-kg.atlassian.net/browse/MB-6419) (проверьте актуальность отдельно
   для более новых проверок — «Исходная оценка», «Sprint»/«Available at...» и «клонирует»).
5. Результат — таблица багов (сначала с замечаниями) с автором бага (поле «Автор»/reporter) и
   отдельными колонками недостающих/лишних полей и недостающих/лишних связей, плюс сводные счётчики.
   Артефакты рана включают `workflow_result.json` и человекочитаемый `bug_backlog_audit_report.txt`.
...
1. Откройте **http://localhost:8000/ui/workflows/uat_bug_test_cases/run**.
...
5. Для каждого нового бага Gemini разбирает описание бага на предусловие/шаги/ожидаемый
...
packages/
  ...
  workflows/uat_bug_test_cases/ — UAT-баги заказчика -> тест-кейсы в Azure DevOps (suite "UAT Bugs" по спринтам) + Gemini
