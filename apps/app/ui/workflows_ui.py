...
def _render_skipped_tests_audit_page(
    *,
    ...
    <html>
    <head>
        <title>Skipped Tests Audit - Triage Bugs Tool</title>
        <style>{_REVIEW_STYLE}</style>
    </head>
    <body>
        {_REVIEW_NAV}
        <div class="container">
            ...
            <p style="margin-bottom: 16px; color: #555;">Сканирует автотесты (*.spec.ts) на предмет `.skip`, `todo`, `жду` или ссылок на баги (MB-XXXX/МВ-ХХХХ), просит Gemini разобраться в настоящей причине каждого пропуска и проверяет упомянутые баги в Jira. Только чтение — ничего не пишет ни в Jira, ни в файлы тестов.</p>
            {error_block}
...
def _render_review_test_cases_page(
    *,
    ...
    <html>
    <head>
        <title>Review Test Cases - Triage Bugs Tool</title>
        <style>{_REVIEW_STYLE}</style>
    </head>
    <body>
        {_REVIEW_NAV}
...
def _render_bug_backlog_audit_page(
    *,
    ...
    <html>
    <head>
        <title>Bug Backlog Audit - Triage Bugs Tool</title>
        <style>{_REVIEW_STYLE}</style>
        <style>
