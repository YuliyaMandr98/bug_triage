...
`todo` / `жду` comment or a bug reference (`MB-1234` / `МВ-1234`), asks Gemini to work
...
Prerequisites: JIRA_*, GEMINI_* configured in .env.
"""
...
from cli_common import build_gemini_client, build_jira_client, load_env
...
async def main() -> None:
    ...
    for name, client in (("Jira", jira_client), ("Gemini", gemini_client)):
